...
class DANDI001754NWBDataSessionFormatRegisteredClass(NWBDataSessionFormatRegisteredClass):
    ...
    Usage:
        ...
        sess = DANDI001754NWBDataSessionFormatRegisteredClass.get_session(
            ...
            override_parameters_flat_keypaths_dict={'nwb.nwb_filename': 'sub-Rat1_ses-19980425T124500_behavior+ecephys.nwb'},
        )
    """
    ...
    _default_nwb_filename = "sub-Rat1_ses-19980425T124500_behavior+ecephys.nwb"
    _dandiset_id = "001754"
    ...
    def build_default_preprocessing_parameters(cls, **kwargs):
        ...
        preprocessing_parameters.nwb = DynamicContainer(unit_location_filter="CA1", nwb_filename=cls._default_nwb_filename, epoch_label_mode="session_type", export_root=None, force_recompute_linear_position=False).override(nwb_overrides)
    ...
    @classmethod
    def _resolve_nwb_filename(cls, basedir, override_parameters_flat_keypaths_dict=None) -> str:
        ...
        if basedir.suffix == ".nwb":
            return basedir.name
        return cls._default_nwb_filename
    ...
    def parse_session_basepath_to_context(cls, basedir, nwb_filename: Optional[str] = None) -> IdentifyingContext:
        ...
        if nwb_filename is None:
            nwb_filename = cls._default_nwb_filename if basedir.suffix != ".nwb" else basedir.name
    ...
    def get_session_name(cls, basedir) -> str:
        basedir = Path(basedir)
        if basedir.suffix == ".nwb":
            return cls._parse_session_id_from_nwb_filename(basedir.name)
        return cls._parse_session_id_from_nwb_filename(cls._default_nwb_filename)
    ...
    def find_nwb_file(cls, basedir, nwb_filename=None) -> Path:
        ...
        if len(candidates) > 1:
            raise FileNotFoundError(f"Multiple NWB files under {basedir}; set nwb.nwb_filename. Candidates: {[a_candidate.name for a_candidate in candidates]}")
        return candidates[0]
    ...
    def _get_nwb_parameters(cls, session):
        ...
        if not hasattr(preprocessing_parameters, "nwb"):
            preprocessing_parameters.nwb = DynamicContainer(unit_location_filter="CA1", nwb_filename=cls._default_nwb_filename, epoch_label_mode="session_type", export_root=None, force_recompute_linear_position=False)
    ...
    def _build_file_prefix(cls, session) -> Path:
        ...
        session_id = session.config.session_name or cls._parse_session_id_from_nwb_filename(nwb_parameters.nwb_filename or cls._default_nwb_filename)
