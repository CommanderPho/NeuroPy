...
from neuropy.utils.mixins.AttrsClassHelpers import BaseAttrsParameterizedParameters
...
class HardcodedProcessingParameters(HDF_SerializationMixin, AttrsBasedClassHelperMixin, BaseAttrsParameterizedParameters):
    """ These are not meant to be changed in a particular instance, rather they're defined for a specific session and provide the defaults, such as the names of the epochs to process
...
class DataSessionFormatBaseRegisteredClass(metaclass=DataSessionFormatRegistryHolder):
    ...
    def _get_session_specific_parameters(cls, session_context: IdentifyingContext) -> HardcodedProcessingParameters:
        """ session-specific type parameters, **not meant to be changed in a particular instance of the class**, rather they're defined for a specific session and provide the defaults, such as the names of the epochs to process
    ...
    def build_default_computation_configs(cls, sess, **kwargs) -> List[DynamicContainer]:
        ...
        kwargs.setdefault('pf_params', PlacefieldComputationParameters(**override_dict({'speed_thresh': 10.0,
                                                                                        #  'grid_bin': cls.compute_position_grid_bin_size(sess.position.x, sess.position.y, num_bins=(64, 64)),
                                                                                        'grid_bin': cls.compute_position_grid_bin_size(sess.position.x, sess.position.y, num_bins=(64, 64)),
                                                                                        'grid_bin_bounds': None, 'smooth': (2.0, 2.0), 'frate_thresh': 1.0, 'time_bin_size': 0.1, 'computation_epochs': None}, ## NOTE: 2025-01-15 06:31 on at least one call (but before the main pf computation) this is hit without using the grid_bin_bounds in the computation, seems to be inferring it from the recorded position info and the desired num_bins in each direction
