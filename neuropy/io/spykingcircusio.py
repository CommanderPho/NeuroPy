...
class SpykingCircusIO:
    ...
    def load_rough_mua(self, mua_filename=None):
        ...
        so it is defined as SpykingCircusIO.rough_mua"""
    ...
    def roughmua2neuroscope(self, chans, shankIDs):
        ...
        files for viewing in neuroscope.  Must run load_rough_mua first. Cluster ids correspond to electrodes.
        Chans = channel numbers in neuroscope."""
        assert self._rough_mua is not None, "Must run load_rough_mua first."
        assert len(chans) == len(
            shankIDs
        ), "electrodes and shankIDs must be the same length"
        rough_mua = self._rough_mua
        ...
        for shank in np.unique(shankIDs):
            ...
            for mua_chan, chan in zip(
                ...
                spikes.extend(rough_mua["spiketimes"]["elec_" + str(mua_chan)][:])
                chanIDs.extend(
                    chan
                    * np.ones_like(
                        rough_mua["spiketimes"]["elec_" + str(mua_chan)][:]
                    )
                )
    ...
    def __setstate__(self, state):
        if "rough_mua" in state and "_rough_mua" not in state:
            state["_rough_mua"] = state.pop("rough_mua")
