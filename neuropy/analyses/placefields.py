...
class PfND(HDFMixin, AttrsBasedClassHelperMixin, ContinuousPeakLocationRepresentingMixin, PeakLocationRepresentingMixin, NeuronUnitSlicableObjectProtocol, BinnedPositionsMixin, PfnConfigMixin, PfnDMixin, PfnDPlottingMixin):
	...
	def to_dict(self):
		...
		# recurse=False: keep nested attrs instances (e.g. Ratemap, NeuronExtendedIdentity) as objects for pickle
		return asdict(self, filter=filter_fn, recurse=False) # serialize using attrs.asdict but exclude the listed properties
	...
	def __setstate__(self, state):
		...
		# Rebuild Ratemap if an older asdict(recurse=True) pickle flattened it to a dict
		ratemap_state = state.get('_ratemap')
		if isinstance(ratemap_state, dict):
			from neuropy.core.ratemap import Ratemap
			ratemap_kwargs = dict(ratemap_state)
			# attrs private fields may appear as _neuron_ids / _metadata in flattened dicts
			if 'neuron_ids' not in ratemap_kwargs and '_neuron_ids' in ratemap_kwargs:
				ratemap_kwargs['neuron_ids'] = ratemap_kwargs.pop('_neuron_ids')
			if 'neuron_extended_ids' not in ratemap_kwargs and '_neuron_extended_ids' in ratemap_kwargs:
				ratemap_kwargs['neuron_extended_ids'] = ratemap_kwargs.pop('_neuron_extended_ids')
			if 'metadata' not in ratemap_kwargs and '_metadata' in ratemap_kwargs:
				ratemap_kwargs['metadata'] = ratemap_kwargs.pop('_metadata')
			ratemap_kwargs.pop('_filename', None)
			state['_ratemap'] = Ratemap(**ratemap_kwargs)
