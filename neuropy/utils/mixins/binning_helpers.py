...
class BinnedPositionComputingMixin:
    """ 
        from neuropy.utils.mixins.binning_helpers import BinnedPositionComputingMixin

    """
    @classmethod
    def perform_add_binned_position_columns(cls, pos_df: pd.DataFrame, xbin_edges=None, ybin_edges=None, active_computation_config=None, debug_print:bool=False, **kwargs) -> pd.DataFrame:
        ...
        if 'y' not in pos_df.columns:
            # Assume 1D:
            ndim = 1
            pos_col_names = kwargs.pop('position_column_names', ('x',))
            binned_col_names = kwargs.pop('binned_column_names', ('binned_x',))
            bin_values = (xbin_edges,)
        else:
            # otherwise assume 2D:
            ndim = 2
            pos_col_names = kwargs.pop('position_column_names', ('x', 'y'))
            binned_col_names = kwargs.pop('binned_column_names', ('binned_x', 'binned_y'))
            bin_values = (xbin_edges, ybin_edges)

        # bin the dataframe's x and y positions into bins, with binned_x and binned_y containing the index of the bin that the given position is contained within.
        pos_df, out_bins, bin_info = build_df_discretized_binned_position_columns(pos_df, bin_values=bin_values, position_column_names=pos_col_names, binned_column_names=binned_col_names, active_computation_config=active_computation_config, force_recompute=kwargs.pop('force_recompute', False), debug_print=debug_print)
    ...
    def adding_binned_position_columns(self, xbin_edges=None, ybin_edges=None, active_computation_config=None, debug_print:bool=False, **kwargs) -> pd.DataFrame:
        ...
        """
        self.df = self.perform_add_binned_position_columns(pos_df=self.df, xbin_edges=xbin_edges, ybin_edges=ybin_edges, active_computation_config=active_computation_config, debug_print=debug_print, **kwargs)
        return self.df
